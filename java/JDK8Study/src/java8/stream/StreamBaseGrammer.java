...
public class StreamBaseGrammer {
    ...
    protected void productStudents() {
        for (int i = 0; i < 10; i++) {
            stuList.add(new Student(String.valueOf((char) (Math.random() * 26 + 98)), (int) (Math.random() * 10 + 10)
    ...
    protected void getMax() {
        Optional<Student> theMaxOne =
                stuList.stream().max((student1, student2) -> student1.getAge().compareTo(student2.getAge()));
    ...
    protected void concat() {
        Stream.concat(stuList.stream().filter(student -> student.getAge() % 2 == 0),
                      stuList.stream().filter(student -> student.getAge() % 2 != 0)).forEach(System.out::println);
        ;
    }
